...
namespace Vildmark.Physics.Common.PhysicsLogic;
...
public sealed class RealExplosion : PhysicsLogic
{
    ...
    public Dictionary<Fixture, Vector2> Activate(Vector2 pos, float radius, float maxForce)
    {
        ...
        Dictionary<Fixture, Vector2> exploded = new(shapeCount + containedShapeCount);
        ...
        for (int i = 0; i < _data.Count; ++i)
        {
                 ...
                 j += offset)
            {
                ...
                foreach (Fixture f in _data[i].Body.FixtureList)
                {
                    ...
                    float impulse = arclen / (MinRays + insertedRays) * maxForce * 180.0f / MathF.PI * (1.0f - Math.Min(1.0f, minlambda));
