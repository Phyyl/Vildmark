...
using Vildmark.Physics.Common.Decomposition.Seidel;
using Point = Vildmark.Physics.Common.Decomposition.Seidel.Point;
...
internal static class SeidelDecomposer
{
    ...
    public static List<Vertices> ConvexPartition(Vertices vertices, float sheer = 0.001f)
    {
        ...
        List<Point> compatList = new(vertices.Count);
        ...
        Triangulator t = new(compatList, sheer);

        List<Vertices> list = new();

        foreach (List<Point> triangle in t.Triangles)
        {
            Vertices outTriangles = new(triangle.Count);
