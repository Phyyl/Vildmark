...
namespace Vildmark.Physics.Common.Decomposition.Seidel;
...
internal class QueryGraph
{
    ...
    {
        YNode yNode = new(edge, Sink.Isink(tList[1]), Sink.Isink(tList[2]));
        XNode qNode = new(edge.Q, yNode, Sink.Isink(tList[3]));
        XNode pNode = new(edge.P, Sink.Isink(tList[0]), qNode);
        Replace(sink, pNode);
    }
    ...
    {
        YNode yNode = new(edge, Sink.Isink(tList[1]), Sink.Isink(tList[2]));
        XNode pNode = new(edge.P, Sink.Isink(tList[0]), yNode);
        Replace(sink, pNode);
    }

    public void Case3(Sink sink, Edge edge, Trapezoid[] tList)
    {
        YNode yNode = new(edge, Sink.Isink(tList[0]), Sink.Isink(tList[1]));
        Replace(sink, yNode);
    }
    ...
    {
        YNode yNode = new(edge, Sink.Isink(tList[0]), Sink.Isink(tList[1]));
        XNode qNode = new(edge.Q, yNode, Sink.Isink(tList[2]));
        Replace(sink, qNode);
    }
}
