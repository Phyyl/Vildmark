...
using Vildmark.Physics.Common.Decomposition;
using Vildmark.Physics.Dynamics;
...
public static class PathManager
{
    ...
    public static void ConvertPathToEdges(Path path, Body body, int subdivisions)
    {
        ...
        if (path.Closed)
        {
            ChainShape chain = new(verts, true);
    ...
    {
        List<RevoluteJoint> joints = new(bodies.Count + 1);

        for (int i = 1; i < bodies.Count; i++)
        {
            RevoluteJoint joint = new(bodies[i], bodies[i - 1], localAnchorA, localAnchorB);
        ...
        {
            RevoluteJoint lastjoint = new(bodies[0], bodies[bodies.Count - 1], localAnchorA, localAnchorB);
