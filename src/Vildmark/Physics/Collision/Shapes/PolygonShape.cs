...
using Vildmark.Physics.Common.ConvexHull;

namespace Vildmark.Physics.Collision.Shapes;
...
public class PolygonShape : Shape
{
    ...
    public Vertices Vertices
    {
        ...
        set
        {
            ...
            for (int i = 0; i < _vertices.Count; ++i)
            {
                ...
                Vector2 temp = new(edge.Y, -edge.X);
                temp.Normalize();
                _normals.Add(temp);
            }
    ...
    public override float ComputeSubmergedArea(ref Vector2 normal, float offset, ref Transform xf, out Vector2 sc)
    {
        ...
        Vector2 intoVec = new(Vertices[intoIndex].X * (1 - intoLambda) + Vertices[intoIndex2].X * intoLambda, Vertices[intoIndex].Y * (1 - intoLambda) + Vertices[intoIndex2].Y * intoLambda);
        Vector2 outoVec = new(Vertices[outoIndex].X * (1 - outoLambda) + Vertices[outoIndex2].X * outoLambda, Vertices[outoIndex].Y * (1 - outoLambda) + Vertices[outoIndex2].Y * outoLambda);
