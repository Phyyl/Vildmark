...
using Vildmark.Physics.Collision;
using Vildmark.Physics.Common;
using Vildmark.Physics.Controllers;
using Vildmark.Physics.Dynamics.Contacts;
using Vildmark.Physics.Dynamics.Joints;

namespace Vildmark.Physics.Dynamics;
...
public partial class World
{
    ...
    public virtual void Add(Body body)
    {
        ...
        if (body.World == this)
            throw new ArgumentException("You are adding the same body more than once.", nameof(body));
        if (body.World != null)
            throw new ArgumentException("body belongs to another world.", nameof(body));
    ...
    public virtual void Remove(Body body)
    {
        ...
        if (body == null)
            throw new ArgumentNullException(nameof(body));
        if (body.World != this)
            throw new ArgumentException("You are removing a body that is not in the simulation.", nameof(body));
    ...
    public void Add(Joint joint)
    {
        ...
        if (joint._world == this)
            throw new ArgumentException("You are adding the same joint more than once.", nameof(joint));
        if (joint._world != null)
            throw new ArgumentException("joint belongs to another world.", nameof(joint));
    ...
    public void Remove(Joint joint)
    {
        ...
        if (joint == null)
            throw new ArgumentNullException(nameof(joint));
        if (joint.World != this)
            throw new ArgumentException("You are removing a joint that is not in the simulation.", nameof(joint));
    ...
    {
        SolverIterations iterations = new();
    ...
    public void Add(Controller controller)
    {
        ...
        if (controller.World == this)
            throw new ArgumentException("You are adding the same controller more than once.", nameof(controller));
        if (controller.World != null)
            throw new ArgumentException("Controller belongs to another world.", nameof(controller));
    ...
    public void Remove(Controller controller)
    {
        ...
        if (controller == null)
            throw new ArgumentNullException(nameof(controller));
        if (controller.World != this)
                throw new ArgumentException("You are removing a controller that is not in the simulation.", nameof(controller));

        controller.World = null;
        ControllerList._list.Remove(controller);
        ControllerList._generationStamp++;

        ControllerRemoved?.Invoke(this, controller);
    }

    public Fixture TestPoint(Vector2 point)
    {
        AABB aabb;
        Vector2 d = new(Settings.Epsilon, Settings.Epsilon);
